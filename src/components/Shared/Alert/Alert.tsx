export const Alert: React.FC<OwnProps> = (props) => {
  const { type, text, textBold } = props;
  return (
    <div
      className={`flex items-center p-4 mb-4 text-sm border rounded-lg ${
        type === "success"
          ? "text-green-800 bg-green-50 border-green-800"
          : "text-error border-error bg-backgroundError"
      } `}
      role="alert"
    >
      {type === "success" ? (
      ...
      <span className="sr-only">{type === "success" ? "Info" : "Error"}</span>
      <div>
        <span className="font-medium">{textBold}</span> {text}
      </div>
    </div>
  );
};
